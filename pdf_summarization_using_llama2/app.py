...
print('Started building the pipeline ------------------------>')
...
template = """
              Please sectionize below medical note :
              ```{text}```
              Output:
           """
prompt = PromptTemplate(template=template, input_variables=["text"])
print('Started building LLM -------------------------------------->')
llm_chain = LLMChain(prompt=prompt, llm=llm)
print('--------------------> LLM Successful <---------------------------------------------')
t1 = datetime.now()
print('Started executing the Prompt :  -------------------------------------->')
